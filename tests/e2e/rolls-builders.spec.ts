...
async function probeRollsBuilders(page: Page): Promise<{ results: FlowResult[] }> {
    const results = await page.evaluate(async (flows: readonly string[]): Promise<FlowResult[]> => {
        ...
        // Real dynamic import of a runtime-variable URL (Foundry-served);
        // `/* @vite-ignore */` keeps the bundler from statically resolving
        // it, and the variable specifier avoids TS2307. Generic over the
        // concrete module interface so each call site supplies its type
        // argument (mirrors weapon-attack.spec.ts).
        const dynImport = async <T>(u: string): Promise<T> => (await import(/* @vite-ignore */ u)) as T;
        const base = '/systems/wh40k-rpg/module';

        // ---------- roll-helpers ----------
        async function probeRollHelpers(): Promise<void> {
            let helpersMod: HelpersModule | null;
            try {
                helpersMod = await dynImport<HelpersModule>(`${base}/rolls/roll-helpers.js`);
            } catch (err) {
        ...
        // ---------- roll-data ----------
        async function probeRollData(): Promise<void> {
            let rollDataMod: RollDataModule | null;
            try {
                rollDataMod = await dynImport<RollDataModule>(`${base}/rolls/roll-data.js`);
            } catch (err) {
        ...
        // ---------- action-data ----------
        async function probeActionData(): Promise<void> {
            let actionDataMod: ActionDataModule | null;
            try {
                actionDataMod = await dynImport<ActionDataModule>(`${base}/rolls/action-data.js`);
            } catch (err) {
        ...
        // ---------- extended-test-data ----------
        async function probeExtendedTestData(): Promise<void> {
            let extMod: ExtendedTestModule | null;
            try {
                extMod = await dynImport<ExtendedTestModule>(`${base}/rolls/extended-test-data.js`);
            } catch (err) {
        ...
        // ---------- damage-data ----------
        async function probeDamageData(): Promise<void> {
            let damageMod: DamageModule | null;
            try {
                damageMod = await dynImport<DamageModule>(`${base}/rolls/damage-data.js`);
            } catch (err) {
        ...
        // ---------- dice/basic-roll ----------
        async function probeBasicRoll(): Promise<void> {
            let basicRollMod: BasicRollModule | null;
            try {
                basicRollMod = await dynImport<BasicRollModule>(`${base}/dice/basic-roll.js`);
            } catch (err) {
