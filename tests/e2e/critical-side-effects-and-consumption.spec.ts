...
async function probeCriticalSideEffects(page: Page): Promise<{ results: FlowResult[] }> {
    const results = await page.evaluate(async (): Promise<FlowResult[]> => {
        ...
        try {
            ...
            record(
                ...
                ok
                    ? null
                    : `rider=${String(rec.riders.helmetTornOff)} report=${String(report.helmetTornOff)} equipped=${String(helmet?.system?.state?.equipped)}`,
            );
        } catch (err) {
