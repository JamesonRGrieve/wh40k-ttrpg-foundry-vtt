...
async function probeMacros(page: Page): Promise<{ results: FlowResult[] }> {
    const results = await page.evaluate(async (): Promise<FlowResult[]> => {
        ...
        try {
            const created = await actorDoc.createEmbeddedDocuments('Item', [{ name: 'macros-probe-talent', type: 'talent', system: { gameSystem: 'dh2' } }]);
