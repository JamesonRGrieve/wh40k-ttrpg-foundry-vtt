...
test.describe.serial('Item Piles integration (Tier B)', () => {
    test('currency registry + wallet paths + pickup preservation against real Documents', async ({ page }) => {
        ...
        for (const r of probe.results) {
            seen.add(r.name);
            if (r.ok) {
                recordCoverage('itempiles.flow', r.name);
            } else {
                failures.push(`${r.name}: ${r.detail ?? 'failed'}`);
            }
        }
        for (const expected of ITEM_PILES_FLOWS) {
        ...
        expect(failures, `${failures.length}/${ITEM_PILES_FLOWS.length} Item Piles flows failed:\n  - ${failures.join('\n  - ')}`).toEqual([]);
    });
});
