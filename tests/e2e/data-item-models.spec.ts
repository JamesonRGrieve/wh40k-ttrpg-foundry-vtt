        ...
        async function probeArmourFlows(): Promise<void> {
            ...
            try {
                ...
                if (armour == null) {
                    ...
                    if (coversAll === false && locCount === 2 && headAP === 4 && armAP === 0 && typeof coverageLabel === 'string' && coverageLabel.length > 0) {
            ...
            try {
                ...
                if (gear == null) {
                    ...
                    if (hasUses === true && exhausted === true && display === '0/5') {
                        ...
                        notes['gear-uses-exhausted'] = `expected hasUses=true exhausted=true display="0/5", got hasUses=${String(hasUses)} exhausted=${String(
                            exhausted,
            ...
            try {
                ...
                if (talent == null) {
                    ...
                    if (has === true && hasSkills && hasTalents && hasTraits) {
                        ...
                        notes['talent-grants-summary'] = `expected has=true and skills/talents/traits entries, got has=${String(has)} summary=${JSON.stringify(
                            arr,
                        )}`;
                    }
                }
            ...
            try {
                ...
                if (talent == null) {
                    ...
                    if (hasSpec === true && fullName === 'probe-talent (Las) x3' && rollable === true) {
                        ...
                        notes['talent-specialization-fullname'] = `expected hasSpec=true fullName="probe-talent (Las) x3" rollable=true, got hasSpec=${String(
                            hasSpec,
            ...
            try {
                ...
                if (ff == null) {
                    ...
                    if (range?.min === 1 && range.max === 15 && checks10 === true && checks16 === false && label === '01-15' && protecting === true) {
                        ...
                        notes[
                            'force-field-overload'
                        ] = `expected range{1,15} checks10=true checks16=false label="01-15" protecting=true, got range=${JSON.stringify(range)} c10=${String(
                            checks10,
                        )} c16=${String(checks16)} label=${JSON.stringify(label)} protecting=${String(protecting)}`;
                    }
                }
            } catch (err) {
            ...
            try {
                ...
                if (trait == null) {
                    ...
                    if (
                        ...
                        notes['trait-level-variable'] = `expected hasLevel=true fullName="probe-trait (X) (4)" isVariable=true label!="", got hasLevel=${String(
                            hasLevel,
            ...
            try {
                ...
                if (mod == null) {
                    ...
                    if (
                        ...
                        notes['weapon-modification-restrictions'] = `expected label⊇Classes:/Types: hasMods=true icon=fa-crosshairs, got label=${JSON.stringify(
                            restrictionsLabel,
            ...
            try {
                ...
                if (ritual == null) {
                    ...
                    if (
                        ...
                        notes['ritual-activation-label'] = `expected type=ceremony testLabel="Ceremony +20" label!="" rollable=true, got type=${JSON.stringify(
                            ritualType,
            ...
            try {
                ...
                if (mutation == null) {
                    ...
                    if (category === 'major' && visible === true && typeof catLabel === 'string' && catLabel.length > 0 && hasVisibleProp) {
                        ...
                        notes['mutation-shape'] = `expected category=major visible=true label!="" chatProps with visible entry, got category=${JSON.stringify(
                            category,
...
test.describe.serial('item DataModel derived-data (Tier B)', () => {
    ...
    test('item DataModels round-trip schema fields and compute derived getters', async ({ page }) => {
        ...
        expect(failures, `${failures.length}/${DATA_ITEM_MODEL_FLOWS.length} data-item-model probes failed:\n  - ${failures.join('\n  - ')}`).toEqual([]);
    });
});
