...
async function probeItemType(page: Page, itemType: string): Promise<ItemTypeProbe> {
    ...
    return { type: itemType, docId: result.docId, sheetRendered: result.sheetRendered, createError: result.createError };
}
...
test.describe.serial('item types (Tier B)', () => {
    test('every item type creates + renders sheet', async ({ page }) => {
        ...
        for (const type of itemTypes) {
            ...
            if (probe.docId === null) {
                const reason = probe.createError ?? 'Item.create returned null';
