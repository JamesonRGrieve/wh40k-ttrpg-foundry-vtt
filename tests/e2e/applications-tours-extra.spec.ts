            ...
            try {
                ...
                if (typeof WH40KTour !== 'function') {
                    ...
                    if (extendsTour && waitResolved && hasReset) {
                        ...
                        notes['tour-wh40k-base-class'] = `extendsTour=${String(extendsTour)} waitResolved=${String(waitResolved)} hasReset=${String(hasReset)}`;
                    }
                }
            } catch (err) {
                ...
                if (live == null) {
                    ...
                    if (typeof WhatIfMixin !== 'function') {
                        ...
                        if (inactiveBefore && activeAfterEnter && after.changeCount === 1 && previewBuilt) {
                            ...
                            notes['whatif-mixin-state'] = `inactiveBefore=${String(inactiveBefore)} activeAfterEnter=${String(activeAfterEnter)} changeCount=${
                                after.changeCount
                            } previewBuilt=${String(previewBuilt)}`;
                        }
                        try {
...
test.describe.serial('applications + tours extra coverage (Tier B)', () => {
    ...
    test('tours + uncovered API mixins / components / dialog render flows', async ({ page }) => {
        ...
        expect(failures, `${failures.length}/${APP_TOURS_EXTRA_FLOWS.length} app-tours-extra probes failed:\n  - ${failures.join('\n  - ')}`).toEqual([]);
    });
});
