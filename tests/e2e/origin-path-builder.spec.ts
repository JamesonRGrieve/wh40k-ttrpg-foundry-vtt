    ...
    try {
        const result = await page.evaluate(
            async ({ moduleUrl }): Promise<{ created: boolean; createError: string | null; flows: FlowResult[] }> => {
                ...
                function probeRendersStepList(): void {
                    try {
                ...
                async function probeAdvanceToNextStep(): Promise<void> {
                    try {
                ...
                async function probeBackToPreviousStep(): Promise<void> {
                    try {
                        const err = await callAction('goToStep', {
                ...
                async function probeSelectOriginCard(): Promise<void> {
                    try {
                ...
                async function probeConfirmOrigin(): Promise<void> {
                    try {
                        const firstStepKey = String(builder.systemConfig?.coreSteps?.[0]?.key ?? 'homeWorld');
                        builder.currentStepIndex = 0;
                ...
                async function probeCancelOrReset(): Promise<void> {
                    try {
                        builder.selections.clear();
                        builder.previewedOrigin = null;
                        builder.currentStepIndex = 0;
                ...
                async function probeCompletesFullPath(): Promise<void> {
                    try {
                        builder.guidedMode = false;
                        builder.selections.clear();
                        builder.previewedOrigin = null;
                ...
                probeRendersStepList();
                await probeAdvanceToNextStep();
                await probeBackToPreviousStep();
                await probeSelectOriginCard();
                await probeConfirmOrigin();
                await probeCancelOrReset();
                await probeCompletesFullPath();
