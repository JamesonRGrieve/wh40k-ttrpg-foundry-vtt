...
async function probeVehicleMethods(page: Page): Promise<{ results: FlowResult[] }> {
...
test.describe.serial('documents/vehicle (Tier B)', () => {
    ...
    test('every WH40KVehicle getter + rollItem branch is exercised', async ({ page }) => {
        ...
        expect(failures, `${failures.length}/${VEHICLE_METHODS_FLOWS.length} vehicle-methods flows failed:\n  - ${failures.join('\n  - ')}`).toEqual([]);
    });
});
