...
async function probeActionManagers(page: import('@playwright/test').Page): Promise<ProbeResult> {
    ...
    try {
        const result = await page.evaluate(async (flowNames: readonly string[]) => {
            ...
            try {
                ...
                try {
                    ...
                    if (!msg?.id) {
                        ...
                        if (el === null) {
                            ...
                            const btn = detached.querySelector('.roll-control__hide-control') as HTMLElement | null;
                            const target = detached.querySelector('#probe-toggle-target') as HTMLElement | null;
                            ...
                            const btn = el.querySelector('.roll-control__hide-control') as HTMLElement | null;
                            const target = el.querySelector('#probe-toggle-target') as HTMLElement | null;
                            if (btn === null || target === null) {
                    ...
                    if (typeof ReloadActionManager?.reloadWeapon !== 'function') {
                        ...
                        if (!live) {
                            ...
                            if (!weapon || !ammo) {
                                ...
                                if (Array.isArray(spare) && spare.length > 0 && hasSpare === true && typeof effective === 'string' && typeof hasCustom === 'boolean') {
                                    const reloadNote = reloadError !== null
                                    ...
                                    setResult(
                                        'reload-action-dispatch',
                                        false,
                                        `helper output unexpected: spare=${Array.isArray(spare) ? spare.length : 'not-array'} hasSpare=${String(hasSpare)} effective=${String(effective)}`,
                                    );
                                }
                            }
                        }
                ...
                try {
                    ...
                    const btn = detached.querySelector('.roll-control__refund') as HTMLElement | null;
                    if (btn === null) {
                        ...
                        if (warnedMessage !== null) {
                            setResult(
                                'chat-card-button-click',
                                true,
                                `handler ran end-to-end and surfaced notification: "${warnedMessage}"`,
                            );
                        } else {
