...
/**
 * `DescriptionTemplate` extends a Foundry-bound `SystemDataModel`, so it can
 ...
 */
const MOD = await import('./description-template').catch((err) => {
    const msg = err instanceof Error ? err.message : String(err);
    console.warn(`DescriptionTemplate could not be imported in this environment: ${msg}`);
    return undefined;
});
...
describe('DescriptionTemplate', () => {
    ...
    //   - description schema (HTMLField) defaults and source-reference fields
    //   - the template is mixed into every item DataModel (weapon, talent, skill, …)
});
