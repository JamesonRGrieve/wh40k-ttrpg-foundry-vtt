...
/** Validation result for a voidcraft build (computed in prepareDerivedData). */
export interface VoidcraftBuildValidation {
...
/**
 * Data model for Voidcraft actors (ship/void scale).
 *
 * Extends the abstract {@link VehicleData} base — it shares the universal
 * classification fields (`size`, `type`, `faction`, `locomotion`, weapons /
 * special-rules text) and overrides `crew` / `speed` / `armour` with its own
 * ship-scale shapes. The full ship-build schema and prepare* logic
 * (hull / void-shields / ship-points / components / weapon-capacity /
 * critical-hit statuses) is unchanged from the former `StarshipData`.
 *
 * Matches template.json "starship" structure.
 */
export default class VoidcraftData extends VehicleData {
    ...
    /** Computed by `prepareDerivedData()` — see `VoidcraftBuildValidation`. */
    declare buildValidation: VoidcraftBuildValidation;
     ...
     * DataModel. Returns a `VoidcraftBuildValidation` for the given budget and
     * iterable of component-like views.
     */
    static validateBuild(budget: number, items: Iterable<{ type: string; system: ShipComponentSystem }>): VoidcraftBuildValidation {
        let spent = 0;
        const covered = new Set<string>();
        for (const item of items) {
            const sys = item.system;
     ...
     * `VoidcraftData.computeAppliedModifiers(items)`.
     *
    ...
    static computeAppliedModifiers(items: Iterable<ShipItemView>): Record<ShipModifierStatKey, ShipAppliedModifier> {
        const out = VoidcraftData._emptyAppliedModifiers();
    ...
    override prepareEmbeddedData(): void {
        ...
        const applied = VoidcraftData.computeAppliedModifiers(itemViews);
