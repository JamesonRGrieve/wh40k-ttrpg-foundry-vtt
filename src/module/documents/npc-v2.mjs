...
export class RogueTraderNPCV2 extends RogueTraderBaseActor {
  ...
  async applyDamage(amount, location = "body", options = {}) {
    ...
    const critical = newWounds === 0 ? 
      this.system.wounds.critical + (this.system.wounds.value - newWounds) : 
      this.system.wounds.critical;
  ...
  async healWounds(amount) {
    const newWounds = Math.min(
      ...
      this.system.wounds.value + amount
    );
  ...
  async duplicate(options = {}) {
    ...
    if (options.randomize) {
      for (const key of Object.keys(data.system.characteristics)) {
        ...
        data.system.characteristics[key].base = Math.max(1, Math.min(99, 
          data.system.characteristics[key].base + variance
        ));
      }
    }
