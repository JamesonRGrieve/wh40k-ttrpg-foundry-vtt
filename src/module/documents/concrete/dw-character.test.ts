...
const MOD = await import('./dw-character').catch((err) => {
    ...
    console.warn(`WH40KDWCharacter could not be imported in this environment: ${msg}`);
    return undefined;
});
...
describe('WH40KDWCharacter', () => {
    it.skipIf(MOD === undefined)('exports a default WH40KDWCharacter class', () => {
        expect(MOD?.default).toBeTruthy();
    });

    it.skipIf(MOD === undefined || BASE_MOD === undefined || ACOLYTE_MOD === undefined)(
        'WH40KDWCharacter extends CharacterDocBase (and therefore WH40KAcolyte)',
        () => {
