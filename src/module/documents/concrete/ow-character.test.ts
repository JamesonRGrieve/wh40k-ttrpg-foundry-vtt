...
const MOD = await import('./ow-character').catch((err) => {
    ...
    console.warn(`WH40KOWCharacter could not be imported in this environment: ${msg}`);
    return undefined;
});
...
describe('WH40KOWCharacter', () => {
    it.skipIf(MOD === undefined)('exports a default WH40KOWCharacter class', () => {
        expect(MOD?.default).toBeTruthy();
    });

    it.skipIf(MOD === undefined || BASE_MOD === undefined || ACOLYTE_MOD === undefined)(
        'WH40KOWCharacter extends CharacterDocBase (and therefore WH40KAcolyte)',
        () => {
