...
export default class StatBlockParser extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  static PATTERNS = {
    ...
    characteristicLong: /\b(Weapon Skill|Ballistic Skill|Strength|Toughness|Agility|Intelligence|Perception|Willpower|Fellowship|Influence)\s*[:=]?\s*(\d{1,3})/gi,
  ...
  static _looksLikeCharacteristicHeader(line) {
    ...
    return required.every((key) => tokens.includes(key));
  }
  ...
  static _parseWeaponEntry(entry) {
    ...
    const segments = details ? details.split(";").map((segment) => segment.trim()).filter(Boolean) : [];
    let range = "Melee";
    let rof = "S/-/-";
    let damage = "1d10";
    let pen = 0;
  ...
  static _parseSkillEntry(entry) {
    ...
    let name = entry.replace(/\([^)]*\)/g, "").replace(/\+\s*\d+/g, "").trim();
  ...
  static _parseNameWithSpecializations(name) {
    ...
    const specializations = specializationText.split(",").map((spec) => spec.trim()).filter(Boolean);
  ...
  static _extractSection(lines, label) {
    ...
    for (let i = 0; i < lines.length; i += 1) {
      ...
      for (let j = i + 1; j < lines.length; j += 1) {
        if (this._isSectionHeader(lines[j])) break;
  ...
  static _cleanEntry(entry) {
    return entry.replace(/\.+$/, "").replace(/\s{2,}/g, " ").trim();
  }
