...
export default class BaseRollDialog extends ApplicationV2Mixin(ApplicationV2 as unknown as ApplicationV2Ctor) {
    ...
    override async _prepareContext(options: ApplicationV2Config.RenderOptions): Promise<Record<string, unknown>> {
        ...
        // `_gameSystemId` (for the per-system `{{themeClassFor}}` on roll prompts) is
        // injected centrally by ApplicationV2Mixin._prepareContext from rollData.sourceActor;
        // spread it back over `...this.rollData` so it survives (#422).
        const gameSystemId = context['_gameSystemId'];
        return {
            ...context,
            ...this.rollData,
