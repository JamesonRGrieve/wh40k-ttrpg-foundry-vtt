...
export default class TextPatternExtractor {
     ...
     */
    static extractSection(lines: string[], header: string, sectionHeaders: string[] = []): string {
     ...
     */
    static parseNumericValue(token: string): number {
        if (!token) return 0;
     ...
     */
    static extractValueTokens(line: string): string[] {
        const tokens = line
            .split(/\s+/)
     ...
     */
    static extractAllPatterns(text: string, pattern: RegExp): RegExpMatchArray[] {
