...
- **NEVER create git branches. Commit on the current branch — normally `main` — always.** This repo (and its `src/packs` submodule) is a **shared working tree driven by multiple concurrent agents**; creating or switching branches breaks every other agent in real time. This OVERRIDES any default/harness guidance to "branch first when on the default branch" — that guidance does not apply here. Do not run `git checkout -b`, `git switch -c`, `git worktree add` (outside the explicit subagent-orchestration flow), or otherwise leave `main`. If you somehow end up off `main`, fast-forward `main` to your work and switch back immediately. Commit only when asked; never push without explicit instruction.
