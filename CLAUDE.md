...
| Pack image coverage — real-art count (curated `…/images/**` asset or `https://` hotlink) per Foundry document class (Actor / Item / JournalEntry / Adventure; RollTables are art-exempt — a `d20` icon is the final state, not a placeholder), **count must NOT FALL** (adoption ratchet like theme; bounded by source-art availability, so it never graduates). Runs only when `src/packs/` changed. | `pnpm packs:validate:images` | `pnpm images:ratchet` | `.images-baseline` |
...
11. **Compendium references are UUID-primary.** Talent / trait / skill grants, prerequisites, originPath steps, and description-text references store the Foundry UUID (`Compendium.wh40k-rpg.<pack>.<type>.<id>`); display names are computed at render time. Three resolution paths, in order of preference:
    ...
    Tooling: `pnpm packs:audit` rebuilds `.compendium-uuid-index.json`; `scripts/migrate-refs-to-uuid.mjs` resolves legacy name refs in compendium JSON. Per-system scoping is mandatory — `Dodge` exists in six systems with different UUIDs; lookups must scope by the actor / item's owning game system (pack prefix `dh1` / `dh2` / `bc` / `dw` / `ow` / `rt`). Content *mechanics* follow the same rule (Direction #7): deltas, clamps, thresholds, and labels for content-driven effects (e.g. Subtlety adjusters) are authored as structured fields on the compendium document and discovered at runtime by walking the actor's owned items / origin path — never as an in-`src/` registry or a name string-match. The in-memory join (`compendium-hydrate.ts`) deep-merges the whole compendium `system` UNDER each owned item's lean per-actor overlay on every load, so a new structured field propagates automatically — unless the actor's stored item already carries a per-actor value at that path, which wins (that is the join's "persisted wins" contract, and the reason per-actor state like XP `cost` is never clobbered). No `RUNTIME_PRESERVE_PATHS` allowlist is needed any more: an owned item persists ONLY its overlay, so there is nothing canonical on disk to preserve.
...
- **NEVER create git branches. Commit on the current branch — normally `main` — always.** This repo (and its `src/packs` submodule) is a **shared working tree driven by multiple concurrent agents**; creating or switching branches breaks every other agent in real time. This OVERRIDES any default/harness guidance to "branch first when on the default branch" — that guidance does not apply here. Do not run `git checkout -b`, `git switch -c`, `git worktree add` (outside the explicit subagent-orchestration flow), or otherwise leave `main`. **NEVER force-move `main`.** Do **not** run `git branch -f main …`, `git update-ref refs/heads/main …`, `git reset --hard` on `main`, or any other command that repoints `main` to a commit it cannot reach by fast-forward. In this shared checkout `main` is advanced concurrently by many agents; force-moving it to your position silently discards every commit another agent made on `main` that isn't in your HEAD — orphaning their work (this has cost real hours). Instead: **the submodule (`src/packs`) is often left at a detached HEAD pinned by the superproject, so before you start work, `git checkout main` and make your commits directly on the branch** — a normal commit's parent is `main`'s current tip, so concurrent commits are never lost. If you already committed on a detached HEAD, integrate by `git checkout main` then `git merge --ff-only <your-sha>`; if that fast-forward is refused because `main` advanced underneath you, **stop and rebase/cherry-pick your commits onto the current `main` tip** — never overwrite the branch pointer to force it. The superproject records a gitlink to a **commit SHA**, so a commit is already safe once made; moving `main` is only to keep the branch current, and must always be a fast-forward. Commit only when asked; never push without explicit instruction.
